...
| POST | `/domain/decision` | 执行企业资本性支出管理系统专属领域规则 |
...
| 字段 | 类型 | 含义 |
| --- | --- | --- |
| `requestNo` | String | 投资申请编号 |
| `requestedAmount` | double | 申请金额 |
| `budgetAvailable` | double | 可用预算 |
| `approvedAmount` | double | 批准金额 |
| `expectedAnnualBenefit` | double | 预计年度收益 |
| `projectProgress` | double | 项目进度(%) |
| `overdueMilestones` | int | 逾期里程碑数 |
| `procurementCompliant` | boolean | 采购流程合规 |
| `acceptancePassed` | boolean | 验收通过 |
| `capitalizationReady` | boolean | 转固资料齐备 |
