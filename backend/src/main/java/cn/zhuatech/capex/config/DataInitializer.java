...
package cn.zhuatech.capex.config;
import cn.zhuatech.capex.domain.DomainCatalog;
import cn.zhuatech.capex.model.*;
import cn.zhuatech.capex.repository.*;
...
@Configuration public class DataInitializer {
 ...
 @Bean CommandLineRunner seed(BusinessRecordRepository records,SystemSettingRepository settings,DomainCatalog catalog) {
  return args -> { if(records.count()>0)return;
            ...
            settings.save(new SystemSetting("recordNoRule", "CAPEX-{YYYY}-{SEQ}"));
   ...
   int sequence=1; for(var module:catalog.modules()) {
    String no="CAPEX-DEMO-"+String.format("%03d",sequence);
